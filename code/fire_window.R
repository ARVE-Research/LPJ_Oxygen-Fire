...
#### CHANGE HERE ####################################
# directories
datapath   <- "F:/LPJLMfire/Output/oxygen_fire/"                                      # path where data is stored
...
# Load map shapefiles
wmap <- readOGR("ne_110m_land/ne_110m_land.shp")
...
for (ox in 1:2){
  ...
  plots[[ox]] = ggplot(mdf[[ox]], aes_(mdf[[ox]]$lon.value,mdf[[ox]]$lat.value,fill=mdf[[ox]]$z.value))+geom_raster(interpolate = TRUE)+
    geom_polygon(data=wmap_df, aes(long,lat, group=group),fill=NA,color="black",lwd=0.2) + 
...
# stack plots
grid.arrange(
  arrangeGrob(plots[[1]] + theme(legend.position = "none", plot.margin=unit(c(0,-0,0,-0.5), "cm")),
              plots[[2]] + theme(legend.position = "none", plot.margin=unit(c(0,0,0,-0.5), "cm")), nrow =2),
...
grid.arrange(
  arrangeGrob(rosplot[[1]] + theme(legend.position = "none", plot.margin=unit(c(0,-0,0,-0.5), "cm")),
              rosplot[[2]] + theme(legend.position = "none", plot.margin=unit(c(0,0,0,-0.5), "cm")), nrow =2),
  leg, nrow=2,heights = c(10,1), top=""
)
